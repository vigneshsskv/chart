...
class FixedTickGenerator<T> implements TickGenerator<T> {
  ...
  @override
  List<T>? generate(List<T> values, SpanBase<T> span) => ticks;
}
...
class ChartAxis<Value> {
  ...
  ChartAxis({
    ...
    this.tickLabelerStyle = const TextStyle(color: Colors.black),
            ...
            spanFn ?? ((values) => ListSpan<Value>(values.toSet().toList())),
        tickGenerator = tickGenerator ?? AutoTickGenerator<Value>();
